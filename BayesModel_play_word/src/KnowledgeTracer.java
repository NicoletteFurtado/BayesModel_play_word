...
public class KnowledgeTracer {
	...
	public SkillSet calculateSkill(StudentLogData studentLogData, SkillSet skillSet, InitMaps initMaps) {
		...
		for (int i = 0; i < studentLogData.getVerificationList().size(); i++) {
			...
			// .intValue()))) {
			System.out.println("entered for step " + studentLogData.getUserStep().get(i) + " for sentence " + sent1);
					...
					for (String word : wordToVerif.keySet()) {
						if (wordToValueMap.keySet().contains(word)) { // some words that the student moved to are
							...
							if (wordToVerif.get(word)) {
								...
								System.err.println(sent1 + studentLogData.getUserStep().get(i));
								System.err.println(wordToVerif);
								System.err.println("shouldn't be herre");
							}
	...
	private void updateSkills(StudentLogData studentLogData, InitMaps initMaps, double newSkill, String word, int count) {
